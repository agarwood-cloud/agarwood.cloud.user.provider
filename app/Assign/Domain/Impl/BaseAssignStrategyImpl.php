...
class BaseAssignStrategyImpl implements BaseAssignStrategy
{
     ...
     * @throws Exception
     */
    public function assignCustomer(int $platformId): int
    {
        // 1. 部门的抢粉信息（该公众号的所有部门的信息, 包含已排好分配粉丝的先后顺序）
        $department = $this->assignSettingRepository->getDepartments($platformId);

        // 如果没有设置抢粉信息，随机分配一个客服
        if (empty($department)) {
            return $this->randomAssign($platformId);
        }

        // 2. 查找最后一个要分配的部门，推断出当前要分配的部门
        $last = $this->assignRepository->lastOfficialAccountObtainFans($platformId); // 最后一个抢粉的信息
     ...
     * @inheritDoc
     */
    public function assignList(int $platformId, int $customerId, int $power = 0): bool
    {
        // 2.检查是否已满足基本的抢粉数
        ...
        // 3. 记录正在抢粉的客服
        $this->pushSetsStatus($platformId, $customerId);
     ...
     * @throws Exception
     */
    public function randomAssign(int $platformId): int
    {
        // 临时的解决方法是，随机分配该公众号一个客服
        $customerAll = array_column($this->assignRepository->getCustomerUuidByServiceUuid($platformId), 'id');
